...
describe CountriesController do
  describe 'GET #index' do
    it 'populates an array of all countries' do
      france = create(:country, name: 'France', population: 689032187932)
      germany = create(:country, name: 'Germany', population: 86741589685)
      get :index
  ...
  describe 'PATCH #update' do
    before :each do
      @country = create(:country, name: 'Germany', population: 689051658)
    end
    ...
    context 'with valid attributes' do
      ...
      it 'changes @country\'s attributes' do
        ...
        expect(@country.population).to eq(678901)
      end
      it 'redirects to the updated country' do
        patch :update, id: @country,
          country: attributes_for(:country)
        expect(reponse).to redirect_to @country
      end
    end
      ...
      it 'does not change the country\'s attributes' do
        patch :update, id: @country,
          country: attributes_for(:country,
                                  ...
                                  population: nil)
        @country.reload
        expect(@country.name).not_to eq('Switzerland')
        expect(@country.population).to eq(678901)
      end
      it 're-renders the :edit template' do
        patch :update, id: @country,
          country: attributes_for(:country, name: nil)
        expect(repsonse).to render_template :edit
      end
    end
  end
